...
export const ViewerOverview: React.FC<ViewerOverviewProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Viewer Header */}
      <div className="flex items-center justify-between bg-slate-900 p-6 rounded-card text-white shadow-md">
        <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <Card className="border-l-4 border-l-blue-600">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Avg Engine Latency</span>
            <Activity className="w-5 h-5 text-blue-600" />
          </div>
          <div className="mt-2 flex items-baseline justify-between">
            <span className="text-2xl font-bold text-slate-900">{stats?.avg_latency_ms || 42} ms</span>
            <span className="text-xs text-blue-600 font-medium">Real-Time Scoring</span>
          </div>
        </Card>
      </div>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {recentHighRisk.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-6 text-slate-400">No transactions recorded</td></tr>
              ) : (
