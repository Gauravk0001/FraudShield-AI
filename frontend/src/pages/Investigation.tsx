...
import {
  ...
  Clock, AlertTriangle, BarChart2, ChevronRight, Zap,
} from 'lucide-react';
  ...
  return (
    <div className="space-y-0.5">
      <div className="flex items-center justify-between text-[11px]">
        <span className="font-mono text-slate-700 truncate">{factor.feature_name}</span>
        <span className={`font-bold ml-2 shrink-0 ${isPositive ? 'text-rose-600' : 'text-emerald-600'}`}>
      ...
      <p className="text-[10px] text-slate-400 leading-tight">{factor.explanation}</p>
    </div>
  );
};
                  ...
                  >
                    <div className="flex items-center justify-between font-semibold text-slate-900 mb-1">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                        inv.status === 'RESOLVED' ? 'bg-emerald-100 text-emerald-800' :
                        inv.status === 'IN_REVIEW' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <div className="text-[11px] text-slate-500 flex justify-between items-center">
                      ...
                      <span className={`flex items-center gap-0.5 ${age >= 5 ? 'text-rose-600 font-semibold' : 'text-slate-400'}`}>
              ...
              <Card>
                ...
                <div className="py-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 bg-slate-50 border border-slate-200 rounded-btn text-xs">
                    ...
                    {selectedInv.transaction && (
                      <>
                        <div className="p-3 bg-slate-50 border border-slate-200 rounded-btn text-xs">
                          ...
                          <span className="font-bold text-slate-900">${selectedInv.transaction.amount.toLocaleString()}</span>
                        </div>
                        <div className="p-3 bg-slate-50 border border-slate-200 rounded-btn text-xs">
                ...
                <Card title="SHAP Risk Factor Evidence" subtitle="ML model feature contributions driving this risk score">
                  <div className="space-y-3 mt-3">
                    ...
                    <div className="pt-2 border-t border-slate-100 text-[10px] text-slate-400 flex items-center gap-1">
                      ...
                      SHAP values from XGBoost + Isolation Forest ensemble
                    </div>
                  </div>
                </Card>
              )}
    ...
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Card className="!p-3 border-l-4 border-l-amber-500">
          <span className="text-[10px] font-bold text-slate-500 uppercase">Open Cases</span>
          <span className="block text-2xl font-extrabold text-amber-700 mt-0.5">{open}</span>
        </Card>
        <Card className="!p-3 border-l-4 border-l-blue-500">
          <span className="text-[10px] font-bold text-slate-500 uppercase">In Review</span>
          <span className="block text-2xl font-extrabold text-blue-700 mt-0.5">{inReview}</span>
        </Card>
        <Card className="!p-3 border-l-4 border-l-emerald-500">
          <span className="text-[10px] font-bold text-slate-500 uppercase">Resolved Today</span>
          <span className="block text-2xl font-extrabold text-emerald-700 mt-0.5">{resolvedToday}</span>
        </Card>
        <Card className={`!p-3 border-l-4 ${agedOut > 0 ? 'border-l-rose-600' : 'border-l-slate-300'}`}>
          <span className="text-[10px] font-bold text-slate-500 uppercase">Age-Out Warnings</span>
          <span className={`block text-2xl font-extrabold mt-0.5 ${agedOut > 0 ? 'text-rose-700' : 'text-slate-500'}`}>
            {agedOut}
          </span>
        </Card>
      </div>
      ...
      <Card title="Full Case Backlog" subtitle="All investigations — sortable by age and status">
        {agedOut > 0 && (
          <div className="mb-3 flex items-center gap-2 px-3 py-2 bg-rose-50 border border-rose-200 rounded-btn text-xs text-rose-700 font-medium">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {investigations.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-8 text-slate-400">No investigations found</td></tr>
              ) : (
                investigations.map((inv) => {
                  const age = daysSince(inv.created_at);
                  const isAged = inv.status !== 'RESOLVED' && age >= 5;
                  return (
                    <tr key={inv.id} className={`transition-colors ${isAged ? 'bg-rose-50/50 hover:bg-rose-50' : 'hover:bg-slate-50'}`}>
                      ...
                      <td className="py-3 px-4 text-slate-600">
                        {inv.assigned_analyst_id
                          ? <span className="flex items-center gap-1"><UserCheck className="w-3.5 h-3.5 text-emerald-600" /> Assigned</span>
                          : <span className="text-slate-400">Unassigned</span>}
                      </td>
                      <td className="py-3 px-4">
                        <span className={`flex items-center gap-1 font-semibold ${isAged ? 'text-rose-700' : 'text-slate-700'}`}>
                          <Clock className="w-3.5 h-3.5" />
                          {age}d {isAged && <AlertTriangle className="w-3.5 h-3.5 text-rose-600" />}
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        {inv.decision
                          ? <span className="text-[10px] bg-purple-100 text-purple-800 px-2 py-0.5 rounded font-bold">{inv.decision}</span>
                          : <span className="text-slate-400">Pending</span>}
                      </td>
                      <td className="py-3 px-4 text-right">
                        <a href="/investigations" className="inline-flex items-center gap-1 text-purple-600 font-semibold hover:text-purple-800">
      ...
      <Card title="Backlog Composition" subtitle="Investigation distribution by status">
        <div className="flex items-end gap-3 pt-2">
          {[
            ...
            return (
              <div key={label} className="flex-1 text-center">
                <div className="text-xs font-bold text-slate-900 mb-1">{pct}%</div>
                <div className="h-20 bg-slate-200 rounded-sm overflow-hidden flex flex-col-reverse">
          ...
          <div className="flex-1 text-center">
            <div className="text-xs font-bold text-rose-700 mb-1">{agedOut > 0 ? agedOut : 0}</div>
            <div className="h-20 bg-slate-200 rounded-sm overflow-hidden flex flex-col-reverse">
              <div
            ...
            <div className="text-[10px] text-slate-500 mt-1">Aged Out</div>
            <div className="text-xs font-bold text-rose-700">{agedOut}</div>
          </div>
        </div>
      </Card>
    </>
...
const ViewerBacklog: React.FC<{ investigations: Investigation[] }> = ({ investigations }) => (
  <>
    <div>
      ...
      <p className="text-xs text-slate-500 mt-0.5">Observer access: no actions permitted.</p>
    </div>
    <Card>
      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-100">
            {investigations.length === 0 ? (
              <tr><td colSpan={4} className="text-center py-6 text-slate-400">No investigations found</td></tr>
            ) : (
              investigations.map((inv) => (
                <tr key={inv.id}>
                  <td className="py-3 px-4 font-mono text-slate-900">#{inv.id.slice(0, 8)}</td>
                  <td className="py-3 px-4">{inv.status}</td>
                  <td className="py-3 px-4 text-slate-500">{new Date(inv.created_at).toLocaleDateString()}</td>
                  <td className="py-3 px-4">{inv.decision ?? '—'}</td>
                </tr>
              ))
            )}
          </tbody>
